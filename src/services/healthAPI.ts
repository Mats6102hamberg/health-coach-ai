...
export class HealthDataAPI {
  ...
  async getSteps(days = 7): Promise<Array<{date: string, steps: number, calories: number}>> {
    // I verkligheten: HealthKit.getStepCount()
    const data: Array<{date: string, steps: number, calories: number}> = [];
  ...
  async getHeartRate(): Promise<{current: number, resting: number, max: number}> {
    // I verkligheten: HealthKit.getHeartRate()
    return {
  ...
  async getSleepData(days = 7): Promise<Array<{date: string, duration: number, quality: number, deepSleep: number}>> {
    // I verkligheten: HealthKit.getSleepAnalysis()
    const data: Array<{date: string, duration: number, quality: number, deepSleep: number}> = [];
    for (let i = days - 1; i >= 0; i--) {
      const date = new Date();
      date.setDate(date.getDate() - i);
      data.push({
        date: date.toISOString().split('T')[0],
        duration: Math.floor(Math.random() * 3) + 6, // 6-9 timmar
        quality: Math.floor(Math.random() * 3) + 1, // 1-4 rating
        deepSleep: Math.floor(Math.random() * 2) + 1 // 1-3 timmar
      });
    }
    return data;
  }
  ...
  async getWeight() {
    // I verkligheten: HealthKit.getBodyMass() eller smart våg
    return {
      weight: 80 + (Math.random() - 0.5) * 4, // Varierar ±2kg
      bodyFat: 15 + Math.random() * 10, // 15-25%
      muscle: 30 + Math.random() * 10, // 30-40%
      timestamp: Date.now()
    };
  }

  async getNutritionData() {
    // I verkligheten: Matfoto-igenkänning eller manuell input
    return {
  ...
  startHeartRateMonitoring(callback: (heartRate: number) => void) {
    // Simulera hjärtfrekvens i realtid
    const interval = setInterval(() => {
      const heartRate = 70 + Math.sin(Date.now() / 1000) * 10 + Math.random() * 5;
...
export class AICoachAPI {
  ...
  constructor() {
    this.openaiKey = import.meta.env.VITE_OPENAI_API_KEY || 'demo-key';
    this.claudeKey = import.meta.env.VITE_ANTHROPIC_API_KEY || 'demo-key';
    this.geminiKey = import.meta.env.VITE_GEMINI_API_KEY || 'demo-key';
  ...
  getAIProvider(): 'openai' | 'claude' | 'gemini' | 'demo' {
